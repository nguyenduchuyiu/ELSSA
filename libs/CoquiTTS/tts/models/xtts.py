...
from CoquiTTS.tts.layers.xtts.gpt import GPT
from CoquiTTS.tts.layers.xtts.hifigan_decoder import HifiDecoder
from CoquiTTS.tts.layers.xtts.stream_generator import init_stream_support
from CoquiTTS.tts.layers.xtts.tokenizer import VoiceBpeTokenizer, split_sentence
from CoquiTTS.tts.layers.xtts.xtts_manager import SpeakerManager, LanguageManager
...
class Xtts(BaseTTS):
    ...
    def get_gpt_cond_latents(self, audio, sr, length: int = 30, chunk_length: int = 6):
        ...
        if self.args.gpt_use_perceiver_resampler:
            ...
            # Fix tensor size mismatch: ensure all style embeddings have the same shape
            if len(style_embs) > 1:
                # Debug: print tensor shapes for troubleshooting
                shapes = [emb.shape for emb in style_embs]
                if not all(shape == shapes[0] for shape in shapes):
                    print(f"⚠️ Warning: Style embedding shape mismatch detected: {shapes}")
                    
                    # Check if only the last dimension differs (sequence length)
                    if all(shape[:-1] == shapes[0][:-1] for shape in shapes):
                        # Find the minimum size along the last dimension  
                        min_size = min(emb.shape[-1] for emb in style_embs)
                        print(f"🔧 Truncating all embeddings to min size: {min_size}")
                        # Truncate all embeddings to the minimum size
                        style_embs = [emb[..., :min_size] for emb in style_embs]
                    else:
                        # More complex shape mismatch - use a different approach
                        print(f"⚠️ Complex shape mismatch detected. Using first embedding only.")
                        style_embs = style_embs[:1]
            elif len(style_embs) == 0:
                raise RuntimeError("No valid style embeddings generated from audio chunks")

            # mean style embedding
            try:
                cond_latent = torch.stack(style_embs).mean(dim=0)
            except RuntimeError as e:
                print(f"❌ Error stacking style embeddings: {e}")
                print(f"📊 Style embedding shapes: {[emb.shape for emb in style_embs]}")
                # Fallback: use only the first embedding
                cond_latent = style_embs[0]
                print(f"🔄 Using first embedding as fallback: {cond_latent.shape}")
        else:
